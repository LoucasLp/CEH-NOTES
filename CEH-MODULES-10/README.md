# 📚 CEH v13 — Module 10 : Denial-of-Service (DoS & DDoS)

> **Certified Ethical Hacker v13 — AI Powered**  
> EC-Council | Formation officielle

---

## 🗂️ Structure des notes

| Fichier | Contenu |
|--------|---------|
| [`01_concepts_fondamentaux.md`](./01_concepts_fondamentaux.md) | Définitions DoS / DDoS, objectifs, impact |
| [`02_categories_attaques.md`](./02_categories_attaques.md) | Volumétriques, Protocoles, Couche applicative |
| [`03_techniques_avancees.md`](./03_techniques_avancees.md) | Botnets, Amplification, Attaques avancées |
| [`04_outils_et_labs.md`](./04_outils_et_labs.md) | Outils offensifs & labs CEHv13 |
| [`05_contre-mesures.md`](./05_contre-mesures.md) | Détection, mitigation, défense |

---

## 🎯 Objectifs du module

À l'issue de ce module, vous serez capable de :

- ✅ Comprendre les concepts fondamentaux des attaques DoS et DDoS
- ✅ Identifier les différentes catégories d'attaques (volumétriques, protocoles, applicatives)
- ✅ Maîtriser les techniques d'attaques avancées (Botnets, Amplification, Réflexion)
- ✅ Utiliser les outils d'audit DoS/DDoS (LOIC, HOIC, Hping3…)
- ✅ Déployer des stratégies de défense et de mitigation efficaces

---

## 📊 Poids dans l'examen CEH (312-50)

```
Module 10 — Denial-of-Service
Poids estimé : ~3-5% des questions de l'examen
Nb de labs officiels : ~6 labs dans la plateforme iLabs/CyberQ
```

---

## 🔗 Ressources complémentaires

- [EC-Council Official CEHv13](https://www.eccouncil.org/train-certify/certified-ethical-hacker-ceh/)
- [CyberQ Platform (Labs)](https://cyberq.eccouncil.org)
- [OWASP DDoS Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/Denial_of_Service_Cheat_Sheet.html)
- [US-CERT DDoS Quick Guide](https://www.cisa.gov/uscert/ncas/tips/ST04-015)

---

*Notes rédigées dans le cadre de la préparation à la certification CEH v13 — EC-Council*  
*Format : Markdown — Compatible GitHub Pages*
